...
class User(db.Model, UserMixin):  # This defines our database
    ...
    resume = db.relationship('Resume', backref='author', lazy=True) # This is a relationship between the user and the application

    def __repr__(self): # This is a magic method that returns a string when we print the object
        return f"User('{self.username}', '{self.email}', '{self.image_file}', '{self.role}', '{self.password}')"
...
class Resume(db.Model):
    ...
    def __repr__(self):
        return f"Resume('{self.work_history}', '{self.education}', '{self.contact_info}')"